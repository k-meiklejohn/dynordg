...
'''
    ...
    parser.add_argument('-a', '--tc_association', help='Integer | The distance in nucleotides for half of a given proportion of scanning ribosomes to reassociate with the ternary complex')
    parser.add_argument('-d', '--sf_dissociation', help='Integer | The distance, measured in nucleotides, for half a given proportion of ribosomes to lose scanning factors after initiation')
    ...
    parser.add_argument('-s', '--scanning_decay', default=0, help='Integer | The halflife of dissociation of scanning ribosomes measured in nucleotides')
    ...
    parser.add_argument('-f', '--fasta', help="Fasta File |  A fasta file to associate to the transcript, overrides --length option")
    ...
    scan_fact  = ScanningFactorDissociation(int(args.sf_dissociation)) if args.sf_dissociation else None
    tran_decay = TranslationDecay(int(args.translation_decay)) if args.translation_decay else None
    tc_assoc   = TernaryComplexAssociation(int(args.tc_association)) if args.tc_association else None
